...
- **hiring-review** -- 職務経歴書・レジュメ（EmDash の `resumes` コレクション。描画は `src/pages/resume/index.astro`）を採用担当者目線（EM + リクルーター）で率直・辛口にレビューする。スキル欄・自己PR・プロジェクト記述のフィードバックを求められた時に使う。
